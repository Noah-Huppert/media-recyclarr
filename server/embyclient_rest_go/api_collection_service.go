...
func (a *CollectionServiceApiService) DeleteCollectionsByIdItems(ctx context.Context, ids string, id string) (*http.Response, error) {
	...
	localVarPath := a.client.cfg.BasePath + "/Collections/{Id}/Items"
...
Requires authentication as user
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param optional nil or *CollectionServiceApiPostCollectionsOpts - Optional Parameters:
     * @param "IsLocked" (optional.Bool) -  Whether or not to lock the new collection.
     * @param "Name" (optional.String) -  The name of the new collection.
     * @param "ParentId" (optional.String) -  Optional - create the collection within a specific folder
...
func (a *CollectionServiceApiService) PostCollectionsByIdItemsDelete(ctx context.Context, ids string, id string) (*http.Response, error) {
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/Collections/{Id}/Items/Delete"
	localVarPath = strings.Replace(localVarPath, "{"+"Id"+"}", fmt.Sprintf("%v", id), -1)
